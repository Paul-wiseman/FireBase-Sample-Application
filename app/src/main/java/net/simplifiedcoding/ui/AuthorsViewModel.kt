...
class AuthorsViewModel : ViewModel() {
    ...
    private val dbAuthorsFirebaseFirestore = FirebaseFirestore.getInstance().collection(NODE_AUTHORS)
    ...
    fun addAuthor(author: Author) {
        dbAuthorsFirebaseFirestore.add(author).addOnSuccessListener {
            _result.value = null
...
//    }
    ...
    fun fetchAuthors() {
        ...
        dbAuthorsFirebaseFirestore.get().addOnSuccessListener { querySnapshot ->
        val authors = mutableListOf<Author>()

            for (authorSnapshort in querySnapshot){
                authors.add(authorSnapshort.toObject(Author::class.java))
            }
            _authors.value = authors
        }.addOnFailureListener {
